...
def parse_args(args=None):
    Description = 'Reformat TADA samplesheet file and check its contents.'
    ...
    while True:
        ...
        if line:
            ...
            if len(lspl) != len(header):
                print_error("Invalid number of columns (minimum = {})!".format(
                    len(header)), line)

            numCols = len([x for x in lspl if x])
            if numCols < 2:
                print_error(
                    "Invalid number of populated columns (minimum = 2)!".format(line))
            ...
            for fastq in fastQFiles:
                if fastq:
                    ...
                    if fastq[-9:] != '.fastq.gz' and fastq[-6:] != '.fq.gz':
                        print_error(
                            "FastQ file does not have extension '.fastq.gz' or '.fq.gz'!", line)
