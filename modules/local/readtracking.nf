process READ_TRACKING {
    ...
    colnames(dadaFs) <- c("dada2.denoised.R1")
    ...
    if ( file.exists("all.dd.R2.RDS") ) {
        ...
        colnames(dadaRs) <- c("dada2.denoised.R2")
    ...
    if (file.exists("all.merged.RDS")) {
        ...
        colnames(mergers) <- c("dada2.mergedpaired")
    ...
    colnames(seqtab.nochim) <- c("dada2.nonchim")
    ...
    if (file.exists("seqtab.${params.id_type}.search_filtered.RDS")) {
        ...
        colnames(seqtab.sf) <- c("mmseqs.searchfilter")
    ...
    if (file.exists("seqtab.tax_filtered.RDS")) {
        ...
        colnames(seqtab.tf) <- c("dada2.taxfilter")
